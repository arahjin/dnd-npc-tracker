...
export default async function OrganisationModal({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!org) notFound();
  if (org.kampagneId !== kampagneId) notFound();
        ...
        <div className="flex flex-wrap items-start gap-6">
          ...
          <div className="flex flex-wrap items-start gap-4 mb-2">
            ...
            {alignColors && org.alignment && (
              <span className="font-cinzel text-sm px-3 py-1 mt-1"
                style={{ background: alignColors.bg, color: alignColors.text, border: `1px solid ${alignColors.border}` }}>
                {org.alignment}
              </span>
            )}
          </div>
