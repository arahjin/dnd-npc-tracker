...
export default function SetupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? t("errorHttp", { status: res.status }));
        setLoading(false);
        return;
      }
      setDone(true);
        ...
        <div style={{ background: "var(--dnd-bg-card)", border: "1px solid var(--dnd-border)" }}>
          ...
          {done ? (
            <div className="p-6 text-center">
              <p className="font-cinzel text-sm" style={{ color: "#4ADE80" }}>{t("doneMessage")}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className="font-cinzel text-xs tracking-[0.15em] uppercase block mb-2" style={{ color: "var(--dnd-label)" }}>{t("nameLabel")}</label>
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} required
                  className="w-full px-4 py-2.5 text-base outline-none" style={inputStyle} />
              </div>
              <div>
                <label className="font-cinzel text-xs tracking-[0.15em] uppercase block mb-2" style={{ color: "var(--dnd-label)" }}>{t("emailLabel")}</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                  className="w-full px-4 py-2.5 text-base outline-none" style={inputStyle} />
              </div>
              <div>
                <label className="font-cinzel text-xs tracking-[0.15em] uppercase block mb-2" style={{ color: "var(--dnd-label)" }}>{t("passwordLabel")}</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required
                  placeholder={t("passwordPlaceholder")} className="w-full px-4 py-2.5 text-base outline-none" style={inputStyle} />
              </div>
