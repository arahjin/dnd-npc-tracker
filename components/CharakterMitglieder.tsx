...
export default function CharakterMitglieder({
  ...
  async function handleRemove(charakterId: string) {
    if (!confirm(tc("removeCharakter"))) return;
    await fetch(`/api/organisationen/${orgId}/charaktermitglieder`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    <div style={{ border: "1px solid var(--dnd-border)", background: "var(--dnd-bg-card)" }}>
      <div className="px-4 py-2" style={{ background: "var(--dnd-red-dark)", borderBottom: "1px solid var(--dnd-border)" }}>
        <h2 className="font-cinzel text-xs tracking-[0.2em] uppercase" style={{ color: "var(--dnd-heading)" }}>
          {t("charaktereTitle", { count: mitglieder.length })}
        </h2>
      </div>

      <div className="divide-y" style={{ borderColor: "#1E1E1E" }}>
        {mitglieder.length === 0 && (
          <p className="px-4 py-4 text-sm" style={{ color: "var(--dnd-text-muted)" }}>{t("emptyCharaktere")}</p>
        )}
        {mitglieder.map((m) => (
      ...
      {verfuegbar.length > 0 && (
        <div className="px-4 py-4 flex flex-wrap gap-2 items-end" style={{ borderTop: "1px solid #1E1E1E" }}>
          <div className="flex-1 min-w-36">
            <p className="font-cinzel text-xs tracking-widest mb-1.5" style={{ color: "var(--dnd-label)" }}>{t("charakterLabel")}</p>
